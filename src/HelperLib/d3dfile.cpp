...
// Desc: 
//-----------------------------------------------------------------------------
const HRESULT CD3DFileObject::SetMeshGeometry( D3DVECTOR* pVertexData,
                                     const DWORD dwNumVertices, DWORD* pFaceData,
                                     const DWORD dwNumFaces )
{
    // Set up vertices
...
// Desc:
//-----------------------------------------------------------------------------
VOID CD3DFileObject::Render( const LPDIRECT3DDEVICE7 pd3dDevice, const BOOL bAlpha, const LPDIRECTDRAWSURFACE7 lpForceTexture)
{
    if( m_bHasMeshData )
    {
        // Render the mesh
    ...
    if( m_pNext )
        m_pNext->Render( pd3dDevice, bAlpha ,lpForceTexture);
...
// Desc:
//-----------------------------------------------------------------------------
const HRESULT CD3DFileObject::ComputeNormals()
{
...
//-----------------------------------------------------------------------------
HRESULT CD3DFile::Load( TCHAR* strFilename,const HMODULE hModule)
{
    HRESULT                 hr;
...
//    CD3DFileObject*         pFrame = NULL;
    ...
    if( FAILED(hr) )
    {
		DXFILELOADRESOURCE r;
		r.hModule=hModule;
		r.lpName=strFilename;
		r.lpType="X";
        hr = pDXFile->CreateEnumObject( &r, DXFILELOAD_FROMRESOURCE,&pEnumObj );
        if( FAILED(hr) )
        {
            pDXFile->Release();
            return hr;
...
//       object
//-----------------------------------------------------------------------------
const HRESULT CD3DFile::Render( const LPDIRECT3DDEVICE7 pd3dDevice )const
{
