...
class CD3DFont
{
    ...
    LPDIRECT3DVERTEXBUFFER7 m_pVB;        // VertexBuffer for rendering text
...
public:
    ...
    HRESULT DrawText( FLOAT x, FLOAT y, const DWORD dwColor, 
                      const char* strText, const DWORD dwFlags=0L );
    ...
    HRESULT InitDeviceObjects( const LPDIRECT3DDEVICE7 pd3dDevice );
    HRESULT RestoreDeviceObjects();
    HRESULT InvalidateDeviceObjects();
    HRESULT DeleteDeviceObjects();

    // Constructor / destructor
    CD3DFont( const TCHAR* strFontName, const DWORD dwHeight, const DWORD dwFlags=0L );
    ~CD3DFont();
};
