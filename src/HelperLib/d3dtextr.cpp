...
/*    m_hbmBitmap = (HBITMAP)LoadImage( GetModuleHandle(NULL), m_strName,
    ...
    m_hbmBitmap = LoadPicture(m_strName);
...
//        if( NULL == ( file = fopen( strPathname, "rb" ) ) )
    ...
    if( NULL == ( strExtension = strrchr( m_strName, TEXT('.') ) ) )
...
/*
	...
	m_pddsSurface=CreateTextureFromBitmap(pd3dDevice,pDD,NULL,m_hbmBitmap,4,TRUE,FALSE,FALSE);
    pDD->Release();
